...
from config_kitti import (
    DATA_DIR, SPLIT, GROUND_CLASSES, IGNORE_CLASSES,
    LABEL_MAPPING_VERSION, LABEL_MAPPING_SOURCE,
    N_POINTS, AUG, MODEL, TRAIN, DEBUG,
)

AUG_CACHE_SCHEMA = "kitti-aug-cache-v4-raw-label-mapping"
...
def map_to_binary(semantic: np.ndarray) -> np.ndarray:
    """
    Map official SemanticKITTI raw semantic IDs to ``{0, 1, 255}``.

    The input is the low 16 bits stored in each ``.label`` value, not the
    learning-map IDs used by some SemanticKITTI training configurations.
    Ground=1 for raw IDs 40/44/48/49/60/72; unlabeled(0) and outlier(1)
    become ignore=255; every other raw semantic ID is non-ground=0.
    """
    semantic = np.asarray(semantic)
    binary = np.zeros(semantic.shape, dtype=np.int32)
    binary[np.isin(semantic, tuple(GROUND_CLASSES))] = 1
    binary[np.isin(semantic, tuple(IGNORE_CLASSES))] = 255
    return binary
...
def preprocess_frame(pts4: np.ndarray, labels: np.ndarray,
    ...
    预处理单帧 KITTI 点云:
      1. 过滤无效点 (x=y=z=0)
      2. 按官方 raw semantic ID 映射为 0/1/255，并在采样前排除 255
    ...
    Returns:
        ...
        labels_bin: (n_pts,)   int32    严格为 [0/1]，与 pts6 一一对应
        若排除无效点和 ignore 标签后不足 100 点，返回 (None, None)
    """
    valid = ~((pts4[:, 0] == 0) & (pts4[:, 1] == 0) &
              (pts4[:, 2] == 0))
    pts4 = pts4[valid]
    labels_bin_all = map_to_binary(labels[valid])

    # Ignore labels must never participate in sampling or training.
    trainable = labels_bin_all != 255
    pts4 = pts4[trainable]
    labels_bin_all = labels_bin_all[trainable]

    if len(pts4) < 100:
        return None, None
    if not np.isin(labels_bin_all, [0, 1]).all():
        raise ValueError("preprocess_frame produced labels outside {0, 1}")
    ...
    if not np.isin(labels_out, [0, 1]).all():
        raise ValueError("preprocess_frame sampled labels outside {0, 1}")
    return pts6, labels_out.astype(np.int32, copy=False)
...
def loadsim_augment(pts6: np.ndarray, labels: np.ndarray,
                    cfg: dict) -> tuple:
    """
    LoadSim physical transform with a strict binary ground-only mask.

    Every LoadSim component (excavation, collapse, surface noise and optional
    smoothing) may change or remove only points whose binary label is 1.
    Non-ground XYZ/features and labels remain byte-for-byte unchanged, apart
    from row compaction when ground points are removed.
    """
    if pts6.ndim != 2 or pts6.shape[1] != 6:
        raise ValueError("loadsim_augment expects pts6 with shape (N, 6)")
    if labels.ndim != 1 or len(pts6) != len(labels):
        raise ValueError("loadsim_augment requires one binary label per point")
    if not np.isin(labels, [0, 1]).all():
        raise ValueError("loadsim_augment requires binary labels {0, 1}")
    if cfg.get("ground_only", True) is not True:
        raise ValueError("LoadSim ground_only must be true")

    pts  = pts6[:, :3].copy()
    nrms = pts6[:, 3:].copy()
    lbls = labels.copy()
    ground_idx = np.flatnonzero(lbls == 1)
    if len(ground_idx) == 0:
        return pts6.copy().astype(np.float32), lbls
    cent = pts[ground_idx].mean(axis=0)
    ...
    for _ in range(random.randint(*cfg["n_ops"])):
        ...
        cent = ground_pts.mean(axis=0)
        proj_ground = (ground_pts - cent) @ d_vec
        p_min, p_max = proj_ground.min(), proj_ground.max()
        if p_max <= p_min:
            continue

        thresh = p_max - random.uniform(*cfg["front_alpha"]) * (p_max - p_min)
        front = ground_idx[proj_ground > thresh]
        if len(front) < 5:
            continue
        ...
        proj_all = (pts - cent) @ d_vec
        dist_s = np.abs((pts - core) @ s_vec)
        ground_mask = lbls == 1
        exc_mask = ground_mask & (dist_s < bw / 2) & (proj_all > thresh)
        take_idx = np.flatnonzero(exc_mask)
        if len(take_idx) == 0:
            continue

        # Ground-only lateral collapse. Non-ground rows are never selected.
        col_idx = np.flatnonzero(
            ground_mask & (dist_s >= bw / 2) &
            (dist_s < bw * 1.2) & (proj_all > thresh))
        if len(col_idx) > 0:
    ...
    # Optional surface noise is also strictly ground-only.
    noise_cfg = cfg.get("surface_noise", {})
    if noise_cfg.get("enabled", False):
        ground_idx = np.flatnonzero(lbls == 1)
        sigma = float(noise_cfg["sigma"])
        clip = float(noise_cfg.get("clip", 3.0 * sigma))
        pts[ground_idx, 2] += np.clip(
            np.random.normal(0.0, sigma, len(ground_idx)), -clip, clip)

    # Optional smoothing updates only ground rows. Formal A4 disables it.
    if cfg.get("smoothing_enabled", False):
        ground_idx = np.flatnonzero(lbls == 1)
        k_s = min(int(cfg["smooth_k"]), max(len(ground_idx) - 1, 0))
        if HAS_SCIPY and k_s > 0:
            tree = cKDTree(pts[ground_idx])
            _, idxs = tree.query(pts[ground_idx], k=k_s + 1)
            nb_mean = pts[ground_idx][idxs[:, 1:]].mean(axis=1)
            factor = float(cfg["smooth_factor"])
            pts[ground_idx] = pts[ground_idx] * (1.0 - factor) + nb_mean * factor

    # Recompute normals only for ground rows so non-ground features are intact.
    ground_idx = np.flatnonzero(lbls == 1)
    if len(ground_idx) > 0:
        nrms[ground_idx] = estimate_normals_fast(pts[ground_idx], k=20)
    pts6_out = np.concatenate([pts, nrms], axis=1).astype(np.float32)
...
def _validate_aug_cache_metadata(data, path: Path) -> None:
    """Reject legacy caches or caches produced by a different label mapping."""
    expected = {
        "cache_schema": AUG_CACHE_SCHEMA,
        "label_mapping_version": LABEL_MAPPING_VERSION,
        "label_mapping_source": LABEL_MAPPING_SOURCE,
    }
    for key, value in expected.items():
        if key not in data.files or str(data[key].item()) != value:
            raise ValueError(
                f"拒绝旧版或标签映射不匹配的缓存 {path}: {key} 必须为 {value!r}")
    expected_ids = {
        "ground_semantic_ids": sorted(GROUND_CLASSES),
        "ignore_semantic_ids": sorted(IGNORE_CLASSES),
    }
    for key, value in expected_ids.items():
        if key not in data.files or data[key].astype(np.int64).tolist() != value:
            raise ValueError(
                f"拒绝标签 ID 合同不匹配的缓存 {path}: {key} 必须为 {value!r}")
...
def build_aug_cache(pairs, strategy: str,
    ...
    if cache_dir is not None:
        ...
        if final_file.exists():
            ...
            with np.load(str(final_file), allow_pickle=False) as data:
                _validate_aug_cache_metadata(data, final_file)
            ...
            if not np.isin(labels_arr, [0, 1]).all():
                raise ValueError(f"缓存含非二分类训练标签: {final_file}")
    ...
    if cache_dir is not None:
        ...
        if progress_file.exists() and partial_file.exists():
            try:
                ...
                with np.load(str(partial_file), allow_pickle=False) as pdata:
                    _validate_aug_cache_metadata(pdata, partial_file)
                    done_pts6 = list(pdata["pts6_list"].astype(np.float32))
                    done_lbls = list(pdata["labels_list"].astype(np.int32))
                if done_lbls and not np.isin(np.stack(done_lbls), [0, 1]).all():
                    raise ValueError("续传缓存含非二分类训练标签")
    ...
    if cache_dir is not None and result:
        ...
        np.savez_compressed(
            ...
            cache_schema=np.array(AUG_CACHE_SCHEMA),
            label_mapping_version=np.array(LABEL_MAPPING_VERSION),
            label_mapping_source=np.array(LABEL_MAPPING_SOURCE),
            ground_semantic_ids=np.array(sorted(GROUND_CLASSES), dtype=np.int32),
            ignore_semantic_ids=np.array(sorted(IGNORE_CLASSES), dtype=np.int32),
        )
...
if HAS_TORCH:
    class KITTIGroundDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            if not np.isin(labels, [0, 1]).all():
                raise ValueError("KITTIGroundDataset requires training labels {0, 1}")
            if self.augment:
