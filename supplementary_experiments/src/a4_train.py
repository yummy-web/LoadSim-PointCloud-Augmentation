"""Formal A4 adapter: three independent SemanticKITTI sequence-00 runs."""
...
def load_contract(config_path: Path, method_arg: str, seed_arg: int,
                  run_dir: Path) -> tuple[dict[str, Any], dict[str, Any], Path]:
    try:
        from generate_configs import validate_config
        with config_path.open("r", encoding="utf-8") as handle:
            run_cfg = validate_config(json.load(handle), config_path.name)
    except Exception as exc:
        raise A4Error(f"invalid run config: {exc}") from exc
    if run_cfg["experiment"] != "A4":
        raise A4Error("a4_train accepts only experiment=A4")
...
def select_split_frames(frames: list[FrameRef], debug: bool,
                        debug_cfg: dict[str, Any]) -> dict[str, list[FrameRef]]:
...
def train_model(torch: Any, nn: Any, PointNetPPKITTI: Any,
    ...
    if epochs < 1 or batch_size < 1 or max_updates < 1 or checkpoint_interval < 1:
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=int(training["lr_step_size"]),
        gamma=float(training["lr_gamma"]))
    amp_enabled = bool(semantic_cfg["training_runtime"]["amp"])
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for batch_index in range(batch_begin, n_batches):
            ...
            if len(indices) < 2:
                raise A4Error("training batch has fewer than two samples")
            ...
            if not torch.isfinite(loss):
                raise A4Error(f"non-finite loss at epoch={epoch} batch={batch_index}")
            ...
            if amp_enabled and scaler.get_scale() < scale_before:
                raise A4Error(
                    f"AMP skipped an optimizer step at epoch={epoch} batch={batch_index}; "
                    "fixed update budget requires 20000 actual parameter updates")
            pred = logits.argmax(dim=1).detach().cpu().numpy()
            labels = y.detach().cpu().numpy()
            accumulator.update(pred, labels, float(loss.item()))
            global_updates += 1
            next_batch = batch_index + 1
            if global_updates % checkpoint_interval == 0:
                atomic_torch_save(latest_path, checkpoint_payload(
                    ...
                    history, accumulator))
        if stop and batch_begin < n_batches:
            break
        if (batch_begin if epoch == start_epoch else 0) >= n_batches:
            pass
        ...
        if improved:
            ...
            best_epoch = epoch + 1
        fresh_accumulator = MetricAccumulator()
        payload = checkpoint_payload(
            torch, fingerprint, run_cfg["run_id"], run_cfg["method"],
            run_cfg["seed"], model, optimizer, scheduler, scaler,
            epoch + 1, 0, global_updates, best_miou, best_epoch,
            history, fresh_accumulator)
        ...
        print(f"epoch {epoch + 1}/{epochs} updates={global_updates} "
    ...
    test_metrics, test_rows = evaluate_split(
        ...
        "test", selected["test"])
    return history, {
        "selection": {"metric": "val.mIoU", "best_epoch": best_epoch,
...
def run(args: argparse.Namespace) -> None:
    ...
    run_cfg, semantic_cfg, yaml_path = load_contract(
        config_path, method, seed, run_dir)
    ...
    selected = select_split_frames(frames, args.debug, semantic_cfg["debug"])
    source_hashes = {
    ...
    metrics.update({
        "schema": "a4-seed-metrics-v1", "status": "completed",
    ...
    atomic_write_json(run_dir / "history.json", {
        "schema": "a4-history-v1", "run_id": run_cfg["run_id"],
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--authorize-formal-run", action="store_true",
                        help="required in addition to LSDA_FORMAL_RUN_AUTHORIZED=YES")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.debug:
        if not args.authorize_formal_run:
            raise SystemExit("formal run denied: pass --authorize-formal-run")
        if os.environ.get("LSDA_FORMAL_RUN_AUTHORIZED") != "YES":
            raise SystemExit("formal run denied: set LSDA_FORMAL_RUN_AUTHORIZED=YES")
    try:
        run(args)
        return 0
